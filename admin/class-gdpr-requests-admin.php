...
class GDPR_Requests_Admin extends GDPR_Requests {
	...
	public function delete_user() {
		if ( ! isset( $_POST['gdpr_delete_user'], $_POST['user_email'], $_POST['index'] ) || ! wp_verify_nonce( sanitize_key( $_POST['gdpr_delete_user'] ), 'gdpr-request-delete-user' ) ) {
			wp_die( esc_html__( 'We could not verify the user email or the security token. Please try again.', 'gdpr' ) );
		}
	...
	public function anonymize_comments() {
		...
		$email         = isset( $_POST['user_email'] ) ? sanitize_email( wp_unslash( $_POST['user_email'] ) ) : '';
		$comment_count = isset( $_POST['comment_count'] ) ? (int) $_POST['comment_count'] : 0;
