...
class GDPR_Requests_Public extends GDPR_Requests {
	...
	public function send_request_email() {
		...
		if ( ! isset( $_POST['type'] ) || ! in_array( wp_unslash( $_POST['type'] ), parent::$allowed_types, true ) ) {
	...
	public function request_confirmed() {
		...
		if ( empty( $meta_key ) || $key !== $meta_key ) {
			return;
		}
		...
		switch ( $type ) {
			case 'delete':
				...
				} elseif ( $this->delete_user( $user, $key ) ) {
					$args = array(
						'user-deleted' => 1,
						'notify'       => 1,
					);
			...
			case 'export-data':
				...
				wp_schedule_single_event( time(), 'mail_export_data', array(
