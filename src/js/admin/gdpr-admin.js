	...
	$(function() {
		...
		$('.add-tab').click(function(e) {
			...
			var tabTemplate = `<div class="postbox" id="cookie-tab-content-${tabID}">
				...
				<input type="hidden" name="gdpr_cookie_popup_content[${tabID}][name]" value="${tabName}" />
				<div class="inside">
					<table class="form-table">
						<tr>
							...
							<td>
								<label class="gdpr-switch">
									<input type="checkbox" name="gdpr_cookie_popup_content[${tabID}][always_active]" id="always-active-${tabID}">
						...
						<tr>
							...
							<td><textarea name="gdpr_cookie_popup_content[${tabID}][how_we_use]" id="tab-how-we-use-${tabID}" cols="53" rows="5"></textarea></td>
						</tr>
						<tr>
							<th><label for="cookies-used-${tabID}">Cookies used by the site</label></th>
							<td>
								<input type="text" name="gdpr_cookie_popup_content[${tabID}][cookies_used]" id="cookies-used-${tabID}" class="regular-text" />
								<br>
		...
		$(document).on('click', '.add-host', function(e) {
			...
			var hostsTemplate = `<div class="postbox">
					...
					<input type="hidden" name="gdpr_cookie_popup_content[${tabID}][hosts][${hostID}][name]" value="${hostID}" />
					<div class="inside">
						<table class="form-table">
							<tr>
								<th><label for="hosts-cookies-used-${hostID}">Cookies used</label></th>
								<td>
									<input type="text" name="gdpr_cookie_popup_content[${tabID}][hosts][${hostID}][cookies_used]" id="hosts-cookies-used-${hostID}" class="regular-text" />
									<br>
									<span class="description">Comma separated list.</span>
								</td>
							</tr>
